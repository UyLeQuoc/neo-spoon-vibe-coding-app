...
export async function newShellProcess(webcontainer: WebContainer, terminal: ITerminal) {
  ...
  output.pipeTo(
    new WritableStream({
      write(data) {
        if (!isInteractive) {
          // biome-ignore lint/suspicious/noControlCharactersInRegex: needed for OSC detection
...
export function sanitizeTerminalOutput(input: string): string {
  ...
  const noOsc = input
    // biome-ignore lint/suspicious/noControlCharactersInRegex: needed for OSC removal
    .replace(/\x1b\](\d+;[^\x07\x1b]*|\d+[^\x07\x1b]*)\x07/g, '')
    .replace(/\](\d+;[^\n]*|\d+[^\n]*)/g, '')
  // Remove ANSI escape sequences
  const noAnsi = noOsc
    // biome-ignore lint/suspicious/noControlCharactersInRegex: needed for ANSI removal
  ...
  const cleaned = formatted
    ...
    .replace(/\s{2,}/g, ' ')
    .trim()
    // biome-ignore lint/suspicious/noControlCharactersInRegex: needed for cleaning
    .replace(/\u0000/g, '')
  return cleaned
}
