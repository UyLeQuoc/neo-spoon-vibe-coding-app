...
import z from 'zod'
import { dbSchema, pendingPaymentsTable, transactionsTable, usersTable } from '~/db/schema'
...
export const verifyPaymentTransactionRoute = factory
  .createApp()
  .post('/verify-payment-transaction', authMiddware, zValidator('json', verifyPaymentInputSchema), async c => {
    ...
    try {
      ...
      try {
        ...
        for (const execution of logData.result.executions) {
          ...
          for (const notification of execution.notifications) {
            ...
            if (normalizedGasTokenHashes.includes(notifContract) && notification.eventname === 'Transfer') {
            ...
            if (normalizedContractHashes.includes(notifContract) && notification.eventname === 'PaymentReceived') {
      ...
      await db.update(usersTable).set({ balance: newBalance }).where(eq(usersTable.address, userAddr)).run()
