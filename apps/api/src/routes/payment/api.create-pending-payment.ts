...
export const createPendingPaymentRoute = factory
  .createApp()
  .post('/create-pending-payment', authMiddware, zValidator('json', createPendingPaymentInputSchema), async c => {
    ...
    try {
      ...
      await db
        ...
        .where(
          and(eq(pendingPaymentsTable.address, userAddr), inArray(pendingPaymentsTable.status, ['pending', 'signed']))
        )
        .run()
